...
export class TempleService {
  ...
  getTemples(): Observable<Temple[]> {
    return this.http
      ...
      .pipe(
        map((response: any[]) =>
          response.map((item) => this.templeAdapter.adapt(item))
        )
      );
  }
  ...
  createTemple(temple: Temple): Observable<Temple> {
    ...
    return this.http
      .post<any>(`${this.apiUrl}`, adaptedTemple)
      .pipe(map((response: any) => this.templeAdapter.adapt(response)));
  }
  ...
  updateTemple(id: string, temple: Temple): Observable<Temple> {
    const adaptedTemple = this.templeAdapter.adapt(temple);
    return this.http
      .put<any>(`${this.apiUrl}${id}`, adaptedTemple)
      .pipe(map((response: any) => this.templeAdapter.adapt(response)));
  }
